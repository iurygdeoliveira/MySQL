...
SELECT A.nome AS 'Nome do Aluno', C.nome AS 'Nome do Curso', AVG(N.nota) AS 'Médias de Notas'
FROM nota N
...
-- Usando a view
SELECT * FROM vw_MEDIAS_DO_CURSO;

-- Alterando tabelas que estão inclusas em uma view
UPDATE aluno set nome = 'Amado Amoroso Amável' WHERE id = 2; 

-- Para alterar uma visão
ALTER VIEW vw_MEDIAS_DO_CURSO AS
SELECT A.nome AS 'Nome do Aluno', AVG(N.nota) AS 'Médias de Notas'
FROM nota N
JOIN resposta R ON R.id = N.resposta_id
JOIN aluno A ON A.id = R.aluno_id
GROUP BY A.nome;
...
CREATE TABLE TBL_VM_MEDIAS_DO_CURSO AS SELECT * FROM vw_MEDIAS_DO_CURSO;
